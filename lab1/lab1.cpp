...
class Point {
	...
	public int getDirectionTo(Point neighbor){
...
class MyAgentProgram implements AgentProgram {
	...
	public Action execute(Percept percept) {
		...
		// Our main decision loop starts here

		if (dirt) {
			return suckDirt();
		}

		if (home && state.isGoingHome) return doNothing();

		if (state.actionQueue.isEmpty() && state.route.isEmpty()) {
			state.route = calculateRoute();
			if (state.route == null) { // means there are no more unknown nodes to find
		...
		if (state.actionQueue.isEmpty()) {
			...
			Point nextPoint = state.route.remove(state.route.size()-1);
	...
	private void goHome() {
		...
		state.route = calculateRoute(new Point(1, 1));
	}

	// BFS to find route to any unknown node
	private ArrayList<Point> calculateRoute() {
		Map<Point, Point> parentLinks = new HashMap<Point, Point>();
		...
		parentLinks.put(startingPoint, null);

		while (!frontier.isEmpty()) {
			Point currentNode = frontier.remove();
			...
			if (currX >= 0 && currY >= 0 && state.world[currX][currY] == state.UNKNOWN) {
				return buildRoute(currentNode, parentLinks);
			}
			...
			for (int dx = -1; dx <= 1; dx++) {
				for (int dy = -1; dy <= 1; dy++) {
					...
					Point neighbor = new Point(currX + dx, currY + dy);
	...
	// BFS to find a route to a specific point
	private ArrayList<Point> calculateRoute(Point endPoint) {
		Map<Point, Point> parentLinks = new HashMap<Point, Point>();
		Queue<Point> frontier = new LinkedList<Point>();
		...
		while (!frontier.isEmpty()) {
			...
			int currY = currentNode.Y;
			// build route to this node if unknown and in bounds
			if (currX >= 0 && currY >= 0 && currX == endPoint.X && currY == endPoint.Y) {
				return buildRoute(currentNode, parentLinks);
			}
	...
	private ArrayList<Point> buildRoute(Point goalNode, Map<Point, Point> parentLinks) {
		ArrayList<Point> route = new ArrayList<Point>();
		route.add(goalNode);

		Point currentPoint = goalNode;
		while ((currentPoint = parentLinks.get(currentPoint)) != null) {
